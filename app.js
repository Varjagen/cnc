...
const CONDITION_GROUPS = {
  negative: [
    ...
    'Unconscious', 'Exhausted', 'On Fire', 'Bleeding', 'Cursed', 'Marked',
  ],
  positive: [
...
function MapCanvas({
  ...
  const startLayerDrag = useCallback((layer, e) => {
    ...
    const onMove = (ev) => {
      ...
      if (d.mode === 'move') {
        setLiveLayer({ id: d.id, x: d.x + (w.x - d.swx), y: d.y + (w.y - d.swy), w: d.w, h: d.h, rotation: d.startRot });
      } else {
        const ang = Math.atan2(w.y - d.cy, w.x - d.cx) * 180 / Math.PI;
        setLiveLayer({ id: d.id, x: d.x, y: d.y, w: d.w, h: d.h, rotation: d.startRot + (ang - d.startAngle) });
      }
    };
    const onUp = () => {
      window.removeEventListener('pointermove', onMove);
      ...
      setLiveLayer(cur => {
        if (cur && d && cur.id === d.id && onLayerTransform) {
          if (d.mode === 'move') onLayerTransform(d.id, { x: Math.round(cur.x), y: Math.round(cur.y) });
          else onLayerTransform(d.id, { rotation: Math.round(cur.rotation) });
        }
        return null;
      });
...
// TOKEN DETAIL PANEL
// ====================================================================
function TokenDetailPanel({ state, token, entity, mode, dispatch, onClose, claimedEntityId, playerActionSender, onLongRest, onShortRest, obfuscateHp }) {
        ...
        {tab === 'core' && (
          <>
            <div className="csheet-identity">
              ...
              <div className="csheet-id-fields">
                ...
                <div className="csheet-row">
                  ...
                  <div className="csheet-field grow"><label>Race / Kin</label><LiveInput value={entity.race || ''} onCommit={(v) => onField({ race: v })} placeholder="e.g. Badger" /></div>
                </div>
                <div className="csheet-row">
            ...
            <div className="csheet-combatrow">
              {[['AC', 'ac', 0, 40, 1], ['Speed', 'speed', 0, 120, 5], ['Init', 'initBonus', -10, 20, 1],
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal slide-up csheet-modal">
        <div className="float-panel-header">
          <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="entity-swatch" style={{ background: entity.color, width: 12, height: 12 }} />
            &#9672; {entity.name} &mdash; Character Sheet
          </span>
...
function HpRoller({ die, conMod, level, value, onResult }) {
  ...
  const go = () => {
    ...
    let running_total = 0; let delay = 0;
    rolls.forEach((r, idx) => {
      const lvl = idx + 1;
      // cycle the face for this step
      tmr.current.push(setTimeout(() => {
        clearInterval(cyc.current);
        if (lvl > 1) cyc.current = setInterval(() => setCur(1 + Math.floor(Math.random() * die)), 70);
        else setCur(die);
      }, delay));
      tmr.current.push(setTimeout(() => {
    ...
    <div className="hp-roller">
      {total != null ? (
        ...
        <>
          ...
          {cur != null && <div className="hp-cur">{cur}</div>}
...
function NewCharacterBuilder({ playerName, busy, onCancel, onCreate, onRoll }) {
  ...
  useEffect(() => { saveCreation({ phase, name, race, level, pool, assign, cls, classLocked, hp }); },
  ...
  const finish = () => {
    ...
    onCreate({ name: name.trim() || `${race} ${cls}`, race, class: cls, level, stats, hp, playerName });
  };

  // ---- Phase chrome ----
  const Stepper = (
    <div className="cc-steps">
      {['Race', 'Stats', 'Class', 'Health'].map((s, i) => (
    ...
    <div className="builder cc">
      ...
      {phase === 1 && (
        <div className="cc-body">
          ...
          <div className="builder-field grow">
            <label>Race</label>
            <select className="class-select" value={race} onChange={e => setRace(e.target.value)}>
              <option value="">— Choose a race —</option>
              {DND_RACES.map(r => <option key={r.name} value={r.name}>{r.name}</option>)}
            </select>
          </div>
          {raceData && (
          ...
          <div className="builder-actions">
            <span />
            <button className="btn primary" disabled={!race} onClick={() => setPhase(2)}>Next: Stats →</button>
          </div>
        </div>
      )}
      ...
      {phase === 2 && (
        <div className="cc-body">
          ...
          <div className="builder-actions">
            <button className="btn ghost" onClick={() => setPhase(1)}>← Race</button>
            <button className="btn primary" disabled={!allRolled || assignedCount < 6} onClick={() => setPhase(3)}>Next: Class →</button>
          </div>
        </div>
      )}
      ...
      {phase === 3 && (
        <div className="cc-body">
          ...
          {!classLocked ? (
            <div className="builder-actions">
              <button className="btn ghost" onClick={() => setPhase(2)}>← Stats</button>
              <button className="btn primary" disabled={!cls}
                onClick={() => { if (confirm(`Lock in ${cls}? You won't be able to change your class after this.`)) setClassLocked(true); }}>🔒 Lock class</button>
            </div>
          ) : (
            <div className="builder-actions">
              <span className="cc-locked">🔒 {cls} locked in.</span>
              <button className="btn primary" onClick={() => setPhase(4)}>Next: Health →</button>
            </div>
          )}
        </div>
      )}
      ...
      {phase === 4 && (
        <div className="cc-body">
          ...
          <div className="cc-summary">
            <b>{name || `${race} ${cls}`}</b> — {race} {cls}, level {level}
            <div className="cc-summary-stats">
              {ABILITY_KEYS.map(k => <span key={k}>{k.toUpperCase()} {stats[k]} ({abilityMod(stats[k]) >= 0 ? '+' : ''}{abilityMod(stats[k])})</span>)}
            </div>
          </div>
          <div className="builder-actions">
            <button className="btn ghost" onClick={() => setPhase(3)}>← Class</button>
...
function ChatPanel({ messages, isDM, myPeerId, defaultName, tokensOnMap = [], recipients = [], onSend }) {
  ...
  const send = () => {
    ...
    if (!text.trim()) return;
    onSend({ text, senderName: myName, whisperTo, whisperToName });
    setInput(''); setError('');
  };
        ...
        <div className="chat-body">
          ...
          <div className="chat-input-row">
            ...
            {isDM ? (
              ...
              <span className="chat-as static" title="You speak as your character">{myName}</span>
            )}
...
function DMRequestsOverlay({ state, dispatch, toast }) {
...
function DMInterface({ state, dispatch, sync, syncStatus, peerCount, peerList, onLogout, roomCode, toast, settings, onSettingsChange, onOpenSettings, showSettings, onCloseSettings }) {
  ...
  const chatTokenNames = useMemo(() => {
    ...
    for (const t of Object.values(state.tokens || {})) {
      if (t.mapId !== currentMapId) continue;
      const e = state.entities[t.entityId];
      if (e && e.name && e.type !== 'Label' && !seen.has(e.name)) { seen.add(e.name); names.push(e.name); }
    }
...
function RevealedMonstersSidebar({ state, currentMapId, ownedEntityIds }) {
  ...
  return (
    <div className={`app-shell ${sicknessWobbleClass}`}>
      ...
      <div className="main player-view">
        ...
        <RevealedMonstersSidebar state={state} currentMapId={currentMapId} ownedEntityIds={ownedEntityIds} />
      </div>
      ...
      <ChatPanel messages={state.chat || []} isDM={false} myPeerId={myPeerId}
        defaultName={displayNameForPeer(state, myPeerId)}
        onSend={({ text }) => playerActionSender({ type: 'chat_send', payload: { text } })} />
    </div>
  );
}
    ...
    switch (action.type) {
      ...
      case 'chat_send': {
        ...
        dispatch({ type: 'CHAT_ADD', message: {
          ...
          text, whisperTo: null, whisperToName: null,
        } });
        return;
      }
