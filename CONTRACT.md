...
| `playerMapOverride` | `string|null` | A player's self-chosen map when not forced. |
...
**overrides** sensitive slices — sanitizes entities (strips DM notes, exact
HP when obfuscated, death saves), hides non-visible tokens, narrows
`reminders`/`forcedViewPerPeer` to that peer, and filters `chat` via
`chatForViewer` (public + their own whispers only). **Spectators** get a
lighter branch. When you add a DM-only field to state, decide whether it
must be stripped here — by default `...state` would leak it.
